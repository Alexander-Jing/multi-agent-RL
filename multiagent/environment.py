...
# 这是所有的environment的初始环境，注意这些环境还有一个底层的动力学和运动学建模环境world类(core.py)
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
        ...
        for i,agent in enumerate(self.agents):
            # 存储下state物理位置 初始
            self.pos_log[i].append(agent.state.p_pos)
    ...
    def _step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            # 存储每一个agent的物理位置
            self.pos_log[i].append([self._get_obs(agent)[2],self._get_obs(agent)[3]])
            obs_n.append(self._get_obs(agent))
            reward_n.append(self._get_reward(agent))
            done_n.append(self._get_done(agent))
            if not agent.adversary:
                self.results[i].append(self._get_result(agent))
            info_n['n'].append(self._get_info(agent))

        # all agents get total reward in cooperative case
        reward = np.sum(reward_n)  # 这里的reward显示的是所有的agent获得的agent
    ...
    def _reset(self):
        ...
        self.agents = self.world.policy_agents
        # 重置results
        self.results = [[] for _ in range(len(self.agents))]
        # 重置p_pos
        self.pos_log = []
        for _ in range(self.n):
            self.pos_log.append([])
    ...
    def _render(self, mode='human', close=True):
        ...
        # create rendering geometry  创建agent对应的圆形
...
class BatchMultiAgentEnv(gym.Env):
    ...
    def _render(self, mode='human', close=True):
        results_n = []
        for env in self.env_batch:
            results_n.append(env.render(mode, close))
        return results_n
